...
select id, substring(nif, 1, length(nif)-1) as nif_digitos, substring(nif, -1) as nif_letra from empleado;
...
select e.*, d.* from empleado e join departamento d on e.id_departamento = d.id;

-- 2. Devuelve un listado con los empleados y los datos de los departamentos donde trabaja cada uno. Ordena el resultado, en primer lugar por el nombre del departamento (en orden alfabético) y en segundo lugar por los apellidos y el nombre de los empleados.
select e.*, d.* from empleado e join departamento d on e.id_departamento = d.id order by d.nombre, e.apellido1, e.apellido2, e.nombre;
...
select distinct d.id, d.nombre, (d.presupuesto - d.gastos) as presupuesto_actual from departamento d join empleado e on d.id = e.id_departamento;
